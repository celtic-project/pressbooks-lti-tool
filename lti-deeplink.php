...
global $wpdb, $lti_tool_data_connector, $lti_tool_session;
...
// Use same LTI version and signature method as platform
    ...
    foreach ($sites as $site) {
        if (in_array($site->blog_id, $available_books)) {
            ...
            if ($selection_interface === 'table') {
                $sql = 'SELECT  p.id, GROUP_CONCAT(t.name) as post_tags' . "\n" .
                    ...
                    'GROUP BY p.id';
                $site_tags = $wpdb->get_results($sql, 'OBJECT_K');
                $hasTags = $hasTags || !empty($site_tags);
                $tags[$site->blog_id] = $site_tags;
            }
    ...
    $css = '  <link rel="stylesheet" href="' . get_admin_url() . 'load-styles.php?load=common">' . "\n";
    ...
    if ($selection_interface === 'table') {
        require_once('lti-deeplink-table.php');
    } else {
        $allowed = array('html' => array('lang' => true), 'title' => array(), 'head' => array(),
            ...
            'span' => array('href' => true, 'class' => true), 'div' => array('class' => true),
